...
import { isReportAuthorized, resolveReportPdfPath } from './lib/reportAccess'
 ...
 * code rather than leaning on them — see `lib/reportAccess.ts` for the
 * matrix and why it is split out:
 *
 ...
 *
 * `pdf_path` is read only to answer "has a PDF been generated yet?" —
 * never to name the object to sign. Before migration 016 it was an
 * ordinary tutor-writable column, so signing it verbatim let a
 * repointed row leak another family's PDF; `resolveReportPdfPath`
 * derives the path instead, from the row's
 * own `student_id`/`academic_year`, which is what `publishFlow.ts`
 * itself signs off of. Kept even though migration 016 also revokes the
 * tutor's write on the column: this is the fix that holds even if a
 * future migration ever reopens it.
 */
export default async (req: Request) => {
  ...
  const { data: report, error: reportError } = await admin
    ...
    .select('id, student_id, academic_year, status, pdf_path')
    .eq('id', reportId)
    .maybeSingle()
