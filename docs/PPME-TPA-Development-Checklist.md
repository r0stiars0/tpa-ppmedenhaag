...
- [x] **An admin can create a person who holds more than one relationship** (ADR-028). The enrolment pickers no longer filter by `users.role`: a tutor or an admin may be named as a child's parent (ADR-024) and a 16+ santri as a class tutor (ADR-020). Until this change every such account in the project existed because SQL put it there, so ADR-024 was a decision the product could not act on and ADR-020(d)'s entitlement was unreachable from the admin side too. The filter was never a control — neither `students.parent_id` nor `classes.tutor_ids` is role-constrained, and only an admin sees these screens — so what changes is reach rather than privilege (DPIA R12). `student` stays out of the parent list deliberately; each option is labelled with the account's current role
  ...
  - [x] Scope switch placement: **not** a tab. A scope is not a destination — pressing it leaves you on the same screen with a different subject — so it never belongs in a bar of `NavLink`s regardless of how much room there is. It renders above the content on mobile (`AppLayout`) and at the right-hand end of the desktop tab row (`DesktopTabs`), which is where the admin "Kelola" entry already lives for the same reason: desktop has the horizontal room. Verified at 390px in both locales; the longest Dutch caption pair is "Mijn groep / Mijn gezin" and neither overflows. (The bottom bar itself does now carry more than five entries — Rapport, plus Beheer for admin — because those *are* destinations; it scrolls horizontally to fit them. See the "Bottom tab nav" line below.)
...
- [x] Bottom tab nav built in confirmed order: Hadir | Tugas | Yanbu'a | Al-Quran | Murajaah — **the same five for every role including admin** since ADR-014. The admin-only tab set that used to *replace* them (Pendaftaran | Kelas | Santri | Rapor) is gone: 5 + 4 will not fit a mobile bottom nav at 44px tap targets, and the 5 above are the prototype-validated set. The enrollment screens sit one level down behind a single "Kelola" entry (a dashboard tile, plus a sixth tab on desktop where there is room), with an `AdminSectionNav` pill strip inside `/admin/*` to move between them. Order unchanged, so no re-validation against the Figma Make prototype was needed — a unit test (`tests/unit/tabs.test.ts`) now pins it. `DesktopTabs` (the `sm:flex` row) now puts the tabs in an `overflow-x-auto` track so they scroll instead of clip: six labels at the larger text a device or a longer translation can impose do not fit a landscape phone (`sm` starts at 640px), and the last one — "Beheer" for admin — was disappearing with no way to reach it. Same pattern `AdminSectionNav` already uses; the scope switch stays a `shrink-0` sibling so it is always visible on the right. **The mobile bar (`BottomTabNav`, `sm:hidden` — the only nav in portrait) got the same treatment and two more destinations**: portrait phones never render `DesktopTabs`, so Rapport (every role reads year-end reports since ADR-014; the `TopNav` `/reports` link is `sm:` and up) and, for admin, Beheer had no home there at all — off the right edge of a five-across bar with no swipe. The bar is now `overflow-x-auto` and its tabs `flex-1 shrink-0`; the active tab is `scrollIntoView`'d on navigation so a deep link to Rapport/Beheer is never hidden. Because it scrolls, the targets are made deliberately large for a big thumb — `min-h-14` (56px, above the 44px floor), `min-w-[5.25rem]`, `text-sm` — at no cost to reach on either end; `AppLayout`'s `pb-24` on `<main>` still clears the taller bar (~57px) with room to spare. The extra two destinations are appended in `bottomNavTabs(role)`, not `NAV_TABS`, so the prototype-validated five and `tabs.test.ts` are untouched; the scope switch is still not a tab (it renders above the content on mobile via `AppLayout`). Verified with Playwright at 390px in both locales as admin (7 tabs, no label clipping, swipe reaches Beheer) and as a parent (6 tabs, Rapport reachable); landscape/desktop unchanged
...
- [x] Attendance check-in UI: 3-state per student (✓ present / clock late / ✕ absent), matching `attendance_status` enum. Since ADR-041 the class-scope register also has a **tutor section** ("Kehadiran guru" / "Aanwezigheid docenten") below the student roster — the same 3-state control for each of the group's tutors, written to `public.tutor_attendance` (a sibling of `attendance`, keyed on the same session) in the same submit; the confirm dialog names the student count and the tutor count separately. Tutor names come from `fn_class_tutors` (`users_self_read` hides other users), which also **omits a student-assistant enrolled in the same group** — their attendance is taken on the student roster, not a second time as a tutor. Read by an admin (any class) and a tutor of the session's class only — never a parent or 16+ student. Offline parity via a `'tutor_attendance'` queue kind. The per-tutor **review timeline** for admins is the immediately following PR (same ADR)
...
**Post-milestone change (TAD ADR-041, migration 022) — PR 1 of 2:** the
attendance register now records **tutor attendance** next to the
students. `public.tutor_attendance` mirrors `attendance` with
`tutor_id → users`, keyed on the same `sessions` row, so it inherits the
ADR-037 schedule stepper and leaves every `attendance` policy and
RLS-01…77 untouched. Read access is narrower than student attendance on
purpose: an admin (any class) and a tutor of the session's class, and
**no parent or student policy at all** — a guardian or 16+ santri sees
zero rows, which is the privacy boundary (DPIA R15). An INSERT's subject
`tutor_id` must be in that class's `tutor_ids`. One new function,
`fn_class_tutors(p_class)` — `security definer`, entitlement folded into
the `WHERE` (the `fn_student_guardians` pattern) — because
`users_self_read` hides other users from a tutor; it also omits a
student-assistant enrolled in the same group, so the register never
records that person as both a student and a tutor for one session
(RLS-87). `TutorAttendanceView`
gains a tutor section (same 3-state control per tutor; a tutor's own
row is editable — presence is not self-evaluation, so no ADR-023
carve-out); the submit confirm names the student and tutor counts
separately; offline parity via a `'tutor_attendance'` queue kind +
`offlineReplay` branch. **No dev-fixture change** — the dev fixture ships
no sessions. Verified: `typecheck` + `typecheck:functions` + `test`
(551) + `build` green; migration 022 applied on a real local Supabase
stack (`supabase db reset`) and the feature click-tested there; the RLS
suite gains RLS-78…87 and a superuser run of `rls.test.sql` reports
`1..350` all green (`supabase test db` also runs in CI's `rls` job). Docs: ADR-041 in the TAD + RLS policy table,
PRD FR-008 + a user story, openapi (`/tutor_attendance` + schemas +
`/rpc/fn_class_tutors`), DPIA R4/R15, both privacy-policy halves, the
