...
`users_self_read` hides other users from a tutor. `TutorAttendanceView`
...
(551) + `build` green; migration 022 applied and its four policies +
`fn_class_tutors` bind-checked against a scratch Postgres; the RLS suite
gains RLS-78…86 (`supabase test db` runs in CI's `rls` job — no local
`supabase` CLI in this environment, and the one running stack is built
from an unrelated branch). Docs: ADR-041 in the TAD + RLS policy table,
