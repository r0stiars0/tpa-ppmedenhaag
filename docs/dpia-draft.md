...
| Data categories | Identity (name, DOB), contact (email via Google account — parents/tutors/16+ students only), educational progress, attendance incl. absence reasons, push subscription tokens. **Pre-registration:** a person who has signed in but has no account yet may submit their own name and a short free-text note to the admin (`registration_requests`, TAD ADR-038); the note is user-typed and may name a child (see retention, and R13) |
| Recipients | Guardians (each child's own active `student_guardians` links — a child may have more than one since ADR-040, and every active guardian has the same access), tutors (own classes only), TPA admins (**all students, all data, read and write** — see §3 and R11); no third-party sharing. One person may hold more than one of these (a tutor whose own child attends); the access is the union, and for *notifications* specifically it is the family relationship alone that decides who is told about a child — TAD ADR-022 |
...
| Retention | Proposed: 3 years post-enrollment, then delete/anonymize. **[IT TEAM]**: confirm. **Registration requests** (`registration_requests`, ADR-038) are shorter-lived: a database trigger deletes the row the moment the admin approves the account, and the admin *reject* action (`reject-registration`, ADR-039) deletes it — via the `auth.users` cascade — when a request will not be approved. A request that is neither approved nor rejected has **no automatic expiry**; it persists until an admin acts. **[IT TEAM]**: decide whether that needs a periodic sweep of untouched requests older than N weeks, or whether relying on the two admin actions is acceptable given how few unregistered sign-ins are expected. |
...
- **Who inside the organization can see what:** access is need-to-know by role and
  ...
  **Notifications follow the same principle, and did not until ADR-022.** Who is
  ...
  than of a filter beside it: recipients come from the child's own active
  `student_guardians` links (TAD ADR-040) and `students.user_id`, neither of
  which a tutor holds for their pupils. In the
  ...
  One limit of the guardian-only confirmation is worth stating plainly, since it is
  inherent rather than fixable: a child under 16 has no credential of their own —
  a `student_guardians` row links a student *record* to a guardian's account and is
  not an account itself, and a Google sign-in with no matching `public.users` row
  reaches the "contact admin" screen and no data (ADR-021). What the app cannot see
  is who is *holding* a signed-in guardian's device. A murajaah confirmation
  therefore means "recorded from a guardian's account", which is the strongest
  claim any password-less household app can make about it. Since ADR-040 a child
  may have more than one guardian; any of them may confirm, and the log records
  which account (`murajaah_log.confirmed_by`).
- **Lawful basis:** **[IT TEAM]** to confirm: consent by a guardian at enrollment
  (recommended for clarity given the community context) vs. legitimate interest.
  For under-16 students, guardian consent is obtained at enrollment regardless.
  Where a child has more than one guardian (ADR-040), each guardian link records
  its own `created_at`, which is the per-guardian consent timestamp.
...
|---|---|---|---|---|
| R1 | Cross-family data leakage (guardian A sees guardian B's child) | Low (if tested) | High | Row Level Security at DB layer for every table; automated RLS test suite in CI asserting negative access (test-plan.md §3); policies reviewed before real data entry. Since ADR-040 "own children" is defined by the `student_guardians` join table rather than a single `parent_id` column, and the negative-access matrix is re-proven for that model against a fixture where one child has two guardians and another has a removed one (test-plan §3.5, RLS-65…77), including that a guardian of one child reads **zero** rows for every other family across every table. |
...
| R13 | **A pre-registration free-text note collects more personal data than needed, from an unverified person** | Medium | Low–Medium | The `registration_requests.description` field (TAD ADR-038) is open text that the form's own prompt invites a person to fill with "which child / why you need access", so it will routinely contain a child's name written by someone the TPA has not yet verified. Controls: (a) the field is **optional** and length-capped (2,000 chars, enforced by a DB `CHECK`), and the form does not ask for anything else; (b) it is readable only by the submitting user and by admins — a database policy, not an app filter (pgTAP RLS-62/RLS-63); (c) it is **deleted on approval** by trigger and **on rejection** by the `reject-registration` Function's `auth.users` cascade (ADR-039), so it does not accumulate; (d) it is never shown to other users, never exported, and never placed in a notification. Residual: a request that is *neither* approved nor rejected retains the note indefinitely (see §2 retention — **[IT TEAM]** to decide on a sweep), and an unverified person can still type a third party's name into a box — mitigated only by the field being optional and admin-only, and by admins being told (privacy policy) what the field is for. **[IT TEAM]**: confirm the lawful basis for this brief pre-approval processing (legitimate interest in vetting access requests) is acceptable. |
| R14 | **A former guardian retains access after a separation or custody change** | Low–Medium | Medium | Since ADR-040 a child may have several guardians, and separations happen — an adult who should no longer see a child's record must lose access cleanly and promptly. Controls: (a) removing a guardian is a `student_guardians` link edit, **admin-only** (`fn_is_admin()`; there is no self-service path), and it takes effect at the **database layer immediately** — `fn_my_children()` filters `unlinked_at IS NULL`, so the removed account's next request returns nothing about the child, across every table (pgTAP RLS-68); (b) no new notification reaches them, because the audience query filters on active links only; (c) the "at least one active guardian" invariant is trigger-enforced, so a removal can never leave a child with no guardian (RLS-73/74). The link **row is retained** with `unlinked_at` set rather than deleted (ADR-040 D7), so there is a record that the access existed and when it ended — the app keeps no general audit log (R11), and for this specific, sensitive change the retained row is that record. Residual, accepted: **existing** notification-centre rows and any `murajaah_log.confirmed_by` entries from the period the person *was* a guardian stay in place (they are historical fact, and removing them would corrupt streak history); and the removal depends on an admin being told to make it. **[IT TEAM]**: add "remove a departed guardian's link" to the same offboarding checklist as tutor/admin departures, and decide whether a court order should also trigger deletion (not just unlinking) of that guardian's historical rows. |
