...
| Retention | Proposed: 3 years post-enrollment, then delete/anonymize. **[IT TEAM]**: confirm. **Registration requests** (`registration_requests`, ADR-038) are shorter-lived: a database trigger deletes the row the moment the admin approves the account, and ADR-039's admin *reject* action deletes it (via the `auth.users` cascade) when a request will not be approved. A request that is neither approved nor rejected has **no automatic expiry** — it persists until an admin acts. **[IT TEAM]**: decide whether that needs a periodic sweep of untouched requests older than N weeks, or whether relying on the reject action is acceptable given how few unregistered sign-ins are expected. |
