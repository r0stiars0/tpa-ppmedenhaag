...
| RLS-43…46 | **`year_end_reports`' write boundary, the sharpest four** (migration 016, TAD ADR-034) — the authoring tutor T1, on a report `yer_tutor_rw`'s row-level rule genuinely lets them reach, is refused a direct `UPDATE … set status = 'published'` (RLS-43), a repoint of `pdf_path` at another family's deterministic path (RLS-44), a rewrite of the attendance snapshot (RLS-45), and a `DELETE` (RLS-46) — all four at the grant, not the row, so each raises `permission denied for table year_end_reports` rather than silently matching 0 rows |
| RLS-47 | Admin is refused the same direct `status` flip. `authenticated` is the one Postgres role every signed-in session connects as, so the column grant refuses admin exactly as it refuses a tutor — publication state belongs to `publish-report` alone |
| RLS-48 | **The anti-regression half, and the more important one** — every `ReportEdit` column still succeeds for the authoring tutor after the same grant that refused RLS-43…46, on a draft and on an already-published report (FR-006's post-publish edit) |
| RLS-49 | Admin can still edit narrative/grades on a published report — exactly ADR-014(e)'s promise, and nothing beyond it |
| RLS-50 | A permitted narrative edit still moves `updated_at`: column-privilege checks look only at a statement's own target-column list, not at what `trg_year_end_reports_touch` writes afterward. Asserted against a sentinel far in the past rather than a before/after `now()` pair, since `now()` is frozen for this file's one enclosing transaction |
| RLS-51 | **GDPR art. 17** — deleting a student still removes their `year_end_reports` rows, even though `authenticated` no longer holds `DELETE` on the table at all. The FK's `on delete cascade` is enforced by Postgres' own referential-integrity trigger on `students`, independent of the deleting role's privileges on the *referencing* table |
| RLS-52 | `service_role` — `publish-report`/`generate-year-end-drafts`' own identity — is untouched: it can still write `status`, `pdf_path` and `published_at` after the revoke, which names only `anon, authenticated` |
| RLS-53 | **The session DELETE boundary, the half that already held** (migration 017, TAD ADR-035) — a tutor's direct `DELETE` on `attendance` destroys nothing, because `attendance` has no DELETE policy for anyone but admin. Asserted first and on its own: it is the intent the next case shows was reachable around |
| RLS-54 | **…and the half that did not.** The same tutor `DELETE`s the *session* instead. `attendance.session_id` is `on delete cascade`, and a referential action is not filtered by the child table's policies, so before 017 this destroyed the register the case above just proved was protected. After 017 the session survives and so does its register — both asserted, since a session that vanished silently would fail the second assertion for the wrong reason |
| RLS-55 | The three verbs a tutor keeps still work — SELECT, INSERT and UPDATE on their own class's sessions. INSERT is the `getOrCreateTodaySession` path (`src/features/attendance/api.ts`), the only way sessions are ever created; nothing in `src/` or `netlify/` issues a `DELETE` against `sessions` at all, which is why removing that verb costs the app nothing |
...
### 3.4 Registration requests (RLS-60…64, ADR-038)

`public.registration_requests` (migration 020), its four policies, the
`security definer` cleanup trigger on `public.users` insert, and the
widened `fn_pending_registrations()`. Each case asserts both the allowed
and the denied/empty side, in the fixture-transaction style the rest of
the suite uses. Fixture additions: an `auth.users` row with **no**
`public.users` row (persona **UR** — unregistered), and a second such row
that already carries a `registration_requests` submission.

- [ ] RLS-60 — UR `INSERT registration_requests` for their **own** `id` is allowed; the same INSERT carrying another user's `id` is refused (`42501`, a `WITH CHECK` refusal, not a silent filter)
...
- [x] `parseEnrolPayload`: lower-cases the e-mail **and the student e-mail**; a non-empty but non-e-mail-shaped student e-mail (`-`, `tidak ada`, …) is treated as **absent** (guardian-only enrolment, never a `502`); trims and length-caps (120) the names; maps `Nederlands`/`nl` → `nl` and anything else → `id`; accepts `YYYY-MM-DD` and a locale date string; rejects a non-date and a future date; records consent when given and still enrols when it is blank / `Tidak` / `false` (ADR-044 — no longer a `400`); drops an over-long `relation` and a blank `student_email` to null; **ignores a payment answer entirely**; flattens a one-element array value (the Apps Script `namedValues` shape)
...
### 4.5k Delete a student (TAD ADR-043)

`tests/unit/adminStudents.test.ts`, against `src/features/admin/api.ts` —
`deleteStudent` is the client call behind the "Hapus" action on
`/admin/students`, the admin's path to remove a duplicate a name-typo
re-submission creates. The RLS backing (admin-only, cascade) is proven
in §3.8 RLS-115.
...
- `tests/unit/reportAccess.test.ts` (11) — `isReportAuthorized` and `resolveReportPdfPath`, extracted out of `report-pdf.mts` for the same reason `publishFlow.ts` gives for its own extraction (TAD ADR-034): so the ordering is testable without a database. The regression case is the point of the file — given a report row whose stored `pdf_path` names another student's object, the path handed to `createSignedUrl` is still the caller's own `{student_id}/{academic_year}.pdf`, derived rather than read — plus `pdf_path === null` resolving to `null` before any signing call, and the admin/tutor/parent/student authorization matrix preserved exactly, including that a tutor with no `class_id` is refused without a database round trip at all
...
| **A child with two guardians → both guardians notified**, each on their own tag (ADR-040); a removed guardian and an unrelated family get nothing | ☑ | ☐ | ☑ (`verify-push.mjs` §2c) |
...
**The multi-guardian fan-out (ADR-040) is ticked for Android Chrome, verified on a real device.** Local stack + `dev-fixture.sql` (Umar has two active `student_guardians` links — Ibu Siti and Bapak Rudi; Zainab has one active and one *removed*), the production build served through `netlify dev --dir dist` on the LAN with a local VAPID pair, the DB webhook pointed at it. A real Android phone (Chrome, the origin added to *"Insecure origins treated as secure"*) subscribed as **Bapak Rudi**. Marking **Umar** absent: the webhook fired, `notify-absence` returned `{"sent":1,"recorded":2}`, the phone showed *"Umar is vandaag niet aanwezig bij TPA"*, and `public.notifications` held one row each for `bapak.rudi` and `ibu.siti`. Marking **Zainab** absent (Rudi's link removed): `{"recorded":1,"skipped":"no push subscription"}`, one row for `ibu.siti` only, **nothing on the phone**. Marking **Salma** absent (guardian is Ustadzah Laila): one row for `ustadzah.laila`, nothing for Rudi or Siti, nothing on the phone. Rudi's real FCM subscription was **retained** after the successful send (a dead endpoint returns `expired:1` and is cleared). `verify-push.mjs` §2c automates the same assertions for a `npm run` against `netlify dev` on localhost. iOS Safari remains unverified — no device.
