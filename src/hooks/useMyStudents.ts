 ...
 * students" (migration 003, policies `students_guardian_read` /
 * `students_self_read`).
 *
 ...
 * of everything the caller may read: for a guardian that is their
 ...
 * were the person's own children. `students_admin_all` has no guardian
 * predicate at all, so for an admin the same query returns every
 * student in the school. `fetchFamilyLinks` asks the narrow question
 * through `fn_my_family_students()` (ADR-040) instead.
 *
